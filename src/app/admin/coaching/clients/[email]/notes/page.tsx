  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const res = await fetch(`/api/admin/coaching/clients/${encodeURIComponent(clientEmail)}`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        if (res.ok) {
          const data = await res.json()
          if (data.client) {
            setClientName(data.client.displayName ?? "")
            setNotes(data.client.privateNotes ?? "")
          }
        }
      } catch { /* handled */ }
      setLoading(false)
          ...
          <div style={{ background: "#161616", border: `1px solid ${border}`, borderRadius: 8, padding: "1.5rem" }}>
            <textarea
              ...
              style={{
                ...
                padding: "1rem", fontFamily: "var(--font-montserrat), sans-serif", fontSize: "0.875rem",
                color: cream, lineHeight: 1.7, resize: "vertical", outline: "none", boxSizing: "border-box",
              }}
            />
