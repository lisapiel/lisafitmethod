...
export default function CoachingAdminShell({ children }: { children: React.ReactNode }) {
    ...
    <>
      <style>{`
        ...
        @media (max-width: 768px) {
          [data-parent="admin-shell"] { background: #faf8f5 !important; color: #1a1a1a !important; }
          [data-parent="admin-shell"] header { display: none !important; }
          [data-parent="admin-shell"] main { padding: 0 !important; max-width: none !important; background: #faf8f5 !important; }
        }
        ...
        /* ── LIGHT-MODE ON MOBILE ────────────────────────────────────────
           Flip the dark admin theme to light on phones using a CSS filter.
           invert(0.94) hue-rotate(180deg) is the standard "preserve hue,
           invert brightness" trick — it turns near-black backgrounds
           into near-white while keeping gold accents warm and readable.
           Images and videos get the same filter twice to cancel out.
           We only apply this to the scroll content — the fixed top bar
           and bottom nav are styled light explicitly so their
           position: fixed / sticky doesn't get broken by the filter's
           stacking context. */
        @media (max-width: 768px) {
          .coaching-mobile-scroll {
            filter: invert(1) hue-rotate(180deg);
            background: #0a0a0a; /* becomes near-white after the filter */
            min-height: calc(100vh - 52px);
            /* prevent filter from bleeding past the wrapping bounds */
            isolation: isolate;
          }
          /* Cancel the filter on assets that should render in their real colors */
          .coaching-mobile-scroll img,
          .coaching-mobile-scroll video,
          .coaching-mobile-scroll iframe,
          .coaching-mobile-scroll .no-invert {
            filter: invert(1) hue-rotate(180deg);
          }
          /* Explicitly-light top bar */
          .coaching-mobile-topbar {
            background: #ffffff !important;
            border-bottom: 1px solid #e8e2dc !important;
          }
          .coaching-mobile-topbar svg,
          .coaching-mobile-topbar svg path,
          .coaching-mobile-topbar svg line,
          .coaching-mobile-topbar svg rect,
          .coaching-mobile-topbar svg circle { stroke: #1a1a1a !important; }
          .coaching-mobile-topbar span {
            color: #1a1a1a !important;
          }
          /* Explicitly-light bottom nav (gold accent already reads well on white) */
          .coaching-mobile-bottomnav {
            background: #ffffff !important;
            border-top: 1px solid #e8e2dc !important;
            box-shadow: 0 -2px 12px rgba(0,0,0,0.04);
          }
          /* Drawer stays comfortable in light mode too */
          .coaching-drawer-panel {
            background: #ffffff !important;
            color: #1a1a1a !important;
            border-left: 1px solid #e8e2dc !important;
          }
          .coaching-drawer-panel a { color: #1a1a1a !important; border-bottom-color: #e8e2dc !important; }
          .coaching-drawer-panel svg { stroke: #1a1a1a !important; }
        }
      `}</style>
      ...
      {drawerOpen && (
        ...
        >
          <div
            className="coaching-drawer-panel"
