  ...
  return (
    ...
    <main style={{ background: "#0a0a0a", color: "#f5f2ee", fontFamily: "var(--font-dm-sans), sans-serif", overflowX: "hidden" }}>
      <style>{`
        ...
        /* Course section — used by both Training (dark) and Nutrition (light) */
        .course-section { padding: 100px 80px; border-top: 1px solid rgba(255,255,255,0.06); }
        .course-section-light { padding: 100px 80px; border-top: 1px solid rgba(0,0,0,0.07); }
        ...
        /* Modules grid — dark variant (Training) */
        ...
        .modules-grid-light { display: grid; grid-template-columns: 1fr 1fr; gap: 1px; background: rgba(0,0,0,0.07); margin-top: 48px; }
        ...
        /* Tracker (LIGHT) */
        .tracker-section { padding: 80px 80px; border-top: 1px solid rgba(0,0,0,0.07); }
        ...
        /* Is This For Me (DARK) */
        .for-section { padding: 100px 80px; border-top: 1px solid rgba(255,255,255,0.06); background: #0a0a0a; }
        @media (max-width: 768px) { .for-section { padding: 72px 28px; } }

        /* FAQ (LIGHT) */
        .faq-section { padding: 100px 80px; border-top: 1px solid rgba(0,0,0,0.07); background: #faf8f5; }
      ...
      {/* NUTRITION FOUNDATIONS — LIGHT */}
      <section id="nutrition" className="course-section-light" style={{ background: "#faf8f5" }}>
        <div className="course-section-inner">
          <div>
            ...
            <div className="courses-divider-light" />
            <p style={{ fontSize: 10, fontWeight: 600, letterSpacing: "0.25em", textTransform: "uppercase", color: "rgba(26,26,26,0.3)", marginBottom: 32 }}>What&apos;s inside</p>
            <div className="modules-grid-light">
              {NUTRITION_MODULES.map((mod) => (
                <div key={mod.num} className="module-card-light">
                  <p style={{ fontFamily: "var(--font-dm-sans), sans-serif", fontSize: 10, letterSpacing: "0.2em", textTransform: "uppercase", color: "rgba(168,137,94,0.7)", marginBottom: 10 }}>{mod.num}</p>
                  <p style={{ fontFamily: "var(--font-playfair), serif", fontSize: `calc(16px * ${hs})`, fontWeight: 700, color: "#1a1a1a", marginBottom: 8, lineHeight: 1.3 }}>{mod.title}</p>
                  <p style={{ fontFamily: "var(--font-dm-sans), sans-serif", fontSize: `calc(12px * ${bs})`, color: "rgba(26,26,26,0.5)", lineHeight: 1.5 }}>{mod.desc}</p>
                </div>
              ))}
            </div>
            <div style={{ marginTop: 40 }}>
              {[
                ...
                <div key={item} style={{ display: "flex", gap: 12, padding: "11px 0", borderBottom: "1px solid rgba(0,0,0,0.07)", fontSize: `calc(13px * ${bs})`, color: "rgba(26,26,26,0.5)", lineHeight: 1.5 }}>
      ...
      {/* TRACKER ADD-ON — LIGHT */}
      <section className="tracker-section" style={{ background: "#f5f2ee" }}>
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          <div style={{ marginBottom: 48 }}>
            <p style={{ fontSize: 10, fontWeight: 600, letterSpacing: "0.3em", textTransform: "uppercase", color: "rgba(168,137,94,0.8)", marginBottom: 16 }}>Optional add-on</p>
            <div style={{ display: "flex", flexWrap: "wrap", alignItems: "baseline", gap: 16, justifyContent: "space-between" }}>
              <h2 style={{ fontFamily: "var(--font-playfair), serif", fontSize: `calc(clamp(24px, 2.5vw, 34px) * ${hs})`, fontWeight: 700, color: "#1a1a1a", lineHeight: 1.2, margin: 0 }}>
                Progress Tracker
              </h2>
              <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
                <span style={{ fontSize: 28, fontWeight: 700, color: "rgba(168,137,94,0.9)", fontFamily: "var(--font-dm-sans), sans-serif", lineHeight: 1 }}>{TRACKER_PRICE_DISPLAY}</span>
                <span style={{ fontSize: 11, color: "rgba(26,26,26,0.35)", fontFamily: "var(--font-dm-sans), sans-serif" }}>once · no subscription</span>
              </div>
            </div>
          </div>
          <div className="tracker-inner">
            {[
              ...
              <div key={title}>
                <p style={{ fontFamily: "var(--font-playfair), serif", fontSize: `calc(16px * ${hs})`, fontWeight: 700, color: "rgba(26,26,26,0.8)", marginBottom: 10 }}>{title}</p>
                <p style={{ fontSize: `calc(13px * ${bs})`, color: "rgba(26,26,26,0.5)", lineHeight: 1.6 }}
                   dangerouslySetInnerHTML={{ __html: body }} />
              </div>
            ))}
          </div>
          <div style={{ marginTop: 40, paddingTop: 32, borderTop: "1px solid rgba(0,0,0,0.08)", display: "flex", flexWrap: "wrap", gap: 16, alignItems: "center" }}>
            <Link href="/checkout" style={{ display: "inline-block", border: "1px solid rgba(168,137,94,0.4)", color: "rgba(26,26,26,0.55)", fontFamily: "var(--font-dm-sans), sans-serif", fontSize: 11, fontWeight: 600, letterSpacing: "0.15em", textTransform: "uppercase", textDecoration: "none", padding: "12px 24px" }}>Add at Checkout</Link>
            <Link href="/tracker-checkout" style={{ color: "rgba(168,137,94,0.75)", fontFamily: "var(--font-dm-sans), sans-serif", fontSize: 11, letterSpacing: "0.1em", textTransform: "uppercase", textDecoration: "none" }}>Tracker only →</Link>
          </div>
        </div>
      </section>
        ...
        <div className="faq-inner">
          ...
          <div>
            {[
              ...
              <div key={q} style={{ padding: "24px 0", borderBottom: "1px solid rgba(0,0,0,0.07)" }}>
                <p style={{ fontFamily: "var(--font-dm-sans), sans-serif", fontSize: `calc(13px * ${bs})`, fontWeight: 600, color: "rgba(26,26,26,0.75)", marginBottom: 10, letterSpacing: "0.03em" }}>{q}</p>
                <p style={{ fontSize: `calc(13px * ${bs})`, color: "rgba(26,26,26,0.5)", lineHeight: 1.7 }}>{a}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* NOT READY — LIGHT (FreeGuideTeaser already uses #f5f2ee) */}
      <FreeGuideTeaser />
